 ...
 */
function send_html_mail(string $to, string $subject, string $html, array $smtp, ?string $cc = null): array {
  ...
  if ($cc !== null) {
    $sendCmd($socket, "RCPT TO:<{$cc}>");
    $ccResp = $readLine($socket);
    if (!$expectCode($ccResp, [250, 251])) {
      fclose($socket);
      return ["ok" => false, "error" => "RCPT TO (CC) falhou: {$ccResp}"];
    }
  }
...
$title = trim((string) ($payload["title"] ?? "Nova mensagem na caixa do Admin"));
$message = trim((string) ($payload["message"] ?? ""));
$senderName = trim((string) ($payload["senderName"] ?? "Usuário InGenium"));
$senderRole = trim((string) ($payload["senderRole"] ?? "student"));
$channel = trim((string) ($payload["channel"] ?? "duvida_sugestao"));
...
foreach ($recipientsRaw as $item) {
  ...
  $fullName = trim((string) ($item["fullName"] ?? "Admin"));
...
foreach ($recipients as $recipient) {
  $safeRecipientName = htmlspecialchars($recipient["fullName"], ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8");
  $safeTitle = htmlspecialchars($title, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8");
  $safeSenderName = htmlspecialchars($senderName, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8");
  $safeSenderRole = htmlspecialchars($senderRole, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8");
  $safeChannel = htmlspecialchars($channel, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8");
  ...
  <div style='font-family:Arial,sans-serif;background:#0a1b33;color:#ffffff;padding:24px;'>
    <div style='max-width:620px;margin:0 auto;background:#10274a;border:1px solid #1f3d6d;border-radius:12px;padding:20px;'>
      ...
      <p style='margin:0 0 10px 0;'>Olá, {$safeRecipientName}.</p>
      <p style='margin:0 0 12px 0;'><strong>Nova mensagem na caixa do Admin</strong></p>
      <p style='margin:0 0 12px 0;'><strong>Assunto:</strong> {$safeTitle}</p>
      <p style='margin:0 0 12px 0;'><strong>Remetente:</strong> {$safeSenderName} ({$safeSenderRole})</p>
      <p style='margin:0 0 12px 0;'><strong>Canal:</strong> {$safeChannel}</p>
      <p style='margin:0 0 12px 0;'><strong>Mensagem:</strong><br>{$safeBody}</p>
      <p style='margin:0;color:#cbd5e1;'>Cópia automática para lucasgonju@gmail.com</p>
    </div>
  </div>";

  $result = send_html_mail($recipient["email"], "InGenium | Nova mensagem para Admin", $html, $smtpCfg, $ccEmail);
  if ($result["ok"]) {
    $sent += 1;
  } else {
    $failed += 1;
