-- Concede +100 XP ao aluno quando preencher o perfil completo pela primeira vez.
...
begin
  case p_event_type
    when 'profile_photo_upload' then
      return 50;
    when 'complete_profile_data' then
      return 100;
...
create or replace function public.award_complete_profile_xp_once()
returns uuid
language plpgsql
security definer
...
begin
  ...
  where e.user_id = v_user_id
    and e.event_type = 'complete_profile_data'
  limit 1;
  ...
  return public.award_xp_event(
    ...
    'complete_profile_data',
    current_date,
    'Completou todos os dados obrigatórios do perfil',
    'profile_complete_data'
  );
end;
$$;

revoke all on function public.award_complete_profile_xp_once() from public;
grant execute on function public.award_complete_profile_xp_once() to authenticated;
grant execute on function public.award_complete_profile_xp_once() to service_role;
